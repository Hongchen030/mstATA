...
test_that("stim_itemcount_con builds correct module-level constraints if num of stimuli in specific modules (all in)", {
  ...
  numItem_sti<-as.vector(table(test_itempool$stimulus))
  # correct operator
  expect_equal(res$operators, rep(rep("<=",n_modules),2*NumStimulus))

  expect_equal(nrow(res$A_binary), 2*n_modules*NumStimulus)
  # RHS = target_num
  expect_equal(res$d, rep(0,2*n_modules*NumStimulus))
  expect_equal(apply(res$A_binary,1,sum),rep(0,2*n_modules*NumStimulus))
  ...
  for(stim_id in seq_len(NumStimulus)){
    ...
    stim_L<-1+2*n_modules*(stim_id-1L)
    stim_U<-1+2*n_modules*(stim_id-1L)+n_modules
    expect_true(all(res$A_binary[stim_L,non_pivot]==-1L))
    expect_true(all(res$A_binary[stim_U,non_pivot]==1L))
    expect_true(res$A_binary[stim_L,pivot]==(num_items-1L))
    expect_true(res$A_binary[stim_U,pivot]==(1L-num_items))
  }
})
