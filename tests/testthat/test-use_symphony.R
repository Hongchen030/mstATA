test_that("use_symphony returns NULL when Rsymphony is not installed", {

  skip_if(
    requireNamespace("Rsymphony", quietly = TRUE),
    "Rsymphony is installed"
  )
  ...
  varname <- "x1"
  modelsense <- "max"
  params <- list(verbosity = -2, time_limit = 99999, gap_limit = c(0.0, 1e-2))
  rowname <- "r1"
