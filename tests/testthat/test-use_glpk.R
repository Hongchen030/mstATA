test_that("use_glpk returns NULL when glpkAPI is not installed", {

  skip_if(
    requireNamespace("Rglpk", quietly = TRUE),
    "The 'Rglpk' package is required. Install it using install.packages('Rglpk')"
  )
  ...
  varname <- "x1"
  modelsense <- "max"
  params <- list(verbose = verbose, presolve = TRUE,tm_limit = time_limit*1000,
...
test_that("Binary knapsack solves correctly", {
  ...
  res <- use_glpk(
    ...
    modelsense = "max",
    params = list(verbose = FALSE, presolve = TRUE,tm_limit = 666*1000,
                  canonicalize_status = FALSE),
    rowname = "con1"
  )
