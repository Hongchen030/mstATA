...
#'   \item \strong{Target plus or minus deviation constraints:} using \code{target} and \code{deviation}
#' }
#'
...
#'
#'   Number of constraints = \strong{(number of modules) x side}
#'
#'   \item \strong{Pathway-level:} when \code{which_pathway} is provided and \code{which_module = NULL}
#'
#'   Number of constraints = \strong{ (number of pathways) x side}
#' }
#'
...
#' @param attribute A string giving the column name in \code{x$ItemPool} containing the
#'   quantitative attribute (e.g., time, difficulty, word count).
#' @param min Optional numeric scalar or vector of lower bounds.
#' @param max Optional numeric scalar or vector of upper bounds.
#' @param target Optional numeric scalar or vector of target values.
#' @param deviation Optional numeric scalar or vector of allowed deviations from the target.
...
#'
#' \itemize{
#'   \item \code{min}: minimum **sum** of item quantitative values
#'   \item \code{max}: maximum **sum** of item quantitative values
#'   \item \code{target}: desired **sum** of item quantitative values
...
#'
#' Let \eqn{q_{i_s}}{q_{i_s}} be the quantitative attribute value of item \eqn{i_s}{i_s}.
#'
...
#'
#'   \item \eqn{m \in r}{m \in r} denote modules belonging to pathway r.
#'
#'   \item \eqn{b_q^{m,\min}}{b_q^{m,\min}} and \eqn{b_q^{m,\max}}{b_q^{m,\max}} specify the lower and upper bounds for
#'   the sum of item quantitative attribute in module m
#'
#'   \item \eqn{b_q^{r,\min}}{b_q^{r,\min}} and \eqn{b_q^{r,\max}}{b_q^{r,\max}} specify the lower and upper bounds for
#'   the sum of item quantitative attribute in pathway r
#' }
#'
#' @return A combined constraint object of class `"mstATA_constraint"`, consisting of one or more quantitative constraints.
#' \describe{
...
#' )
#'
#' @seealso \code{\link{test_itemquant_con}}
#' @export

test_itemquant_range_con <- function(x, attribute,
                                                   min = NULL, max = NULL,
  ...
  if (xor(has_min, has_max) && !has_targ) {
    ...
    val<-expand_target_to_matrix(input = val,levels = NULL,total_rows = total_rows,row_ids = row_ids)[,1]
    return(test_itemquant_con(x = x, attribute = attribute,
                              operator = op, target_value = val,
                              which_module = which_module,
                              which_pathway = which_pathway))
  }

  bounds <- check_target_bounds(levels = attribute,min = min,max = max,target = target,deviation = deviation,
                                total_rows = total_rows,row_ids = row_ids)

  constraints <- list()
  min<-bounds$min[,1]
  max<-bounds$max[,1]
  constraints[[1]] <- test_itemquant_con(x = x,attribute = attribute,
                                                       operator = ">=",target_value = min,
                                                       which_module = which_module,
                                                       which_pathway = which_pathway)
  constraints[[2]] <- test_itemquant_con(x = x,attribute = attribute,
                                                       operator = "<=",target_value = max,
                                                       which_module = which_module,
                                                       which_pathway = which_pathway)
