...
#'
#' Let \eqn{q_{i_s}}{q_{i_s}} be the quantitative attribute value of item \eqn{i_s}{i_s}. Then for
#' module m, the conditional constraint is:
#'
#' \deqn{
#' \sum_{i_s=1}^{I_s} q_{i_s} x_{i_s,m}
#'    \substack{\le \\ \ge \\ =}
#' b_{q}^{m}   x_{i_s^{*},m}, s = 1, \ldots, S - 1.
#' }
#'
#' Here:
#'
...
#'
#'   \item \eqn{b_{q}}{b_{q}^{m}} is the required quantitative bound for the sum of
