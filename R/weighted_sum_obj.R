...
#'
#' @param x An object of class "mstATA_design" created by \link{mst_design}().
#' @param multiple_terms A list containing **two or more** objective components.
#'   Each element must be either:
#'   \describe{
#'     \item{An objective term}{created using \link{objective_term}().}
#'     \item{A compiled objective}{produced by a multi-objective formulation
#'       such as \link{maximin_obj}(), \link{capped_maximin_obj}(),
#'       \link{minimax_obj}(), or \link{goal_programming_obj}().}
#'   }
...
#' \eqn{a^\top x}{a^\top x}, constructed by \link{objective_term}().
#'
...
#' variable \eqn{y_k}{y_k} for each term and optimizes a weighted sum:
#'
#' \deqn{\min \sum_{k=1}^K w_k  y_k}
#' or
#' \deqn{\max \sum_{k=1}^K w_k y_k}
#'
#' where \eqn{w_k > 0}{w_k > 0} are user-specified or default weights.
#'
...
#' a **minimization** form internally when mixed directions are present.
#'
#' For any term that is originally a maximization objective, its score is
#' multiplied by -1. For example, if:
#'
#' \itemize{
#'   \item \eqn{y_1}{y_1} and \eqn{y_2}{y_2} are to be minimized, and
#'   \item \eqn{y_3}{y_3} is to be maximized,
#' }
#'
#' then the combined objective becomes:
#'
#' \deqn{\min ( w_1 y_1 + w_2 y_2 - w_3 y_3)}
#'
...
#' **How to use:**
#' \itemize{
#'   \item Define individual objective terms using \link{objective_term}().
#'         Objective terms may be entirely relative, entirely absolute,
#'         or a mixture of both.
#'   \item Combine them directly using \link{weighted_sum_obj}().
#'   \item Alternatively, first construct compiled objectives using
#'         \link{maximin_obj}(), \link{capped_maximin_obj}(),
#'         \link{minimax_obj}(), or \link{goal_programming_obj}(),
#'         and then combine multiple compiled objectives using
#'         \link{weighted_sum_obj}().
#' }
#'
#' @section Mathematical Formulation:
#'
#' For each objective term \eqn{k = 1, \ldots, K}{k = 1, \ldots, K}:
#'
#' \deqn{y_k = a_k^\top x}
#'
#' where:
...
#' \itemize{
#'   \item \eqn{a_k}{a_k} is the coefficient vector defined by the attribute,
#'         level, and application scope of the objective term;
#'   \item \eqn{x}{x} is the vector of binary item-module-panel decision variables;
#'   \item the objective value is \eqn{y_k = a_k^\top x}{y_k = a_k^\top x}.
#' }
#'
...
#'   \item \eqn{a_k}{a_k} is the coefficient vector defined by the attribute,
#'         level, and application scope of the objective term;
#'   \item \eqn{g_k}{g_k} is the target value specified by the test developer;
#'   \item the objective minimizes deviation from the target:
#'         \eqn{|a_k^\top x - g_k|}{|a_k^\top x - g_k|};
...
#'
#' \deqn{\min \sum_{k=1}^K w_k \tilde{y}_k}
#'
#' where:
#' \eqn{\tilde{y}}{\tilde{y}_k = y_k} if term k is a minimization objective.
#' \eqn{\tilde{y}}{\tilde{y}_k = -y_k} if term k is a maximization objective.
#'
#'
#' @return A list of class "compiled_objective".
#' \describe{
...
#' \emph{Linear models for optimal test design} (pp. 68--69).
...
weighted_sum_obj<-function(x,multiple_terms,strategy_args = list()){
  ...
  return(compile_objective(x = x,obj_set = multiple_terms,
                           strategy = "weighted_sum",strategy_args = strategy_args))
}
