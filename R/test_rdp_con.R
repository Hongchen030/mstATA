...
#'
#' For each routing decision point \eqn{\theta_{rdp}}{\theta_{rdp}}, this function constrains
#' the difference in test information functions (TIFs) between adjacent
#' next-stage modules to be within a specified tolerance:
#' \deqn{| I_{m_j}(\theta_r) - I_{m_{j+1}}(\theta_r) | \le \tau}
#' where \eqn{\tau}{\tau} is the information tolerance.
#'
...
#' \eqn{m = 1, \ldots, M}{m = 1, \ldots, M} denote the module index. Let
#' \eqn{x_{i_s m}}{x_{i_s m}} be a binary decision variable indicating whether item
#' \eqn{i_s}{i_s} from stimulus \eqn{s}{s} is selected into module \eqn{m}{m}.
#'
#' Let \eqn{\theta_{rdp}}{\theta_{rdp}} denote a routing decision point (RDP) at which examinees
#' are routed to different modules in the next stage of the test. The item
#' information function of item \eqn{i_s}{i_s} at ability level \eqn{\theta_r}{\theta_r} is
#' denoted by \eqn{I_{i_s}}{I_{i_s}(\theta_{rdp})}, and the corresponding module-level test
...
#' }
#'
#' Consider routing after stage \eqn{k}{k}. Let \eqn{\mathcal{M}}{\mathcal{M}_{k+1}} denote the
#' ordered set of modules at stage \eqn{k+1}{k+1}. For each pair of adjacent modules
#' \eqn{(m_j, m_{j+1}}{(m_j, m_{j+1})} in \eqn{\mathcal{M}}{\mathcal{M}_{k+1}}, a routing decision point
#' \eqn{\theta_{r_j}}{\theta_{r_j}} is specified. The routing information balance constraint
...
#' \eqn{\delta > 0}{\delta > 0}:
#' \deqn{
#'   | I_{m_j}(\theta_{r_j}) - I_{m_{j+1}}(\theta_{r_j}) |
#'   \le \delta.
#' }
#'
