...
#'
#'   \item \code{weights} -- Optional positive numeric vector of length(multiple_term)
#'    specifying the relative importance of each
...
#' where:
#' \itemize{
#'   \item \eqn{a_k^\top x}{a_k^\top x} is the linear score defined by the objective term;
...
#'
#'   Introduces one nonnegative deviation variable \eqn{d_k \ge 0}{d_k \ge 0} for each term:
#'
#'   \deqn{
#'     |a_k^\top x - g_k| \le d_k.
#'   }
...
#'   \eqn{d_k^+ \ge 0}{d_k^+ \ge 0} and \eqn{d_k^- \ge 0}{d_k^- \ge 0}:
#'
#'   \deqn{
...
#'
#'   with constraints:
#'   \deqn{a_k^\top x - g_k \le d_k^+}
#'
#'   \deqn{g_k - a_k^\top x \le d_k^-}
#'
#'   The objective minimizes:
#'   \deqn{
...
#' \itemize{
#'   \item \eqn{d_k^+}{d_k^+} measures how much objective \eqn{k}{k} exceeds its target;
#'   \item \eqn{d_k^-}{d_k^-} measures how much objective \eqn{k}{k} falls short;
#'   \item \eqn{d_k}{d_k} (one-dev) measures total absolute deviation.
#' }
#'
...
#'   \item \eqn{g_k}{g_k} is the target value.
#' }
#'
#' Auxiliary deviation variables are introduced to linearize
#' \eqn{|a_k^\top x - g_k|}{|a_k^\top x - g_k|}, and the solver minimizes a weighted sum of these
#' deviations.
#'
