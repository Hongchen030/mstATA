...
#' the largest absolute mismatch between each linear score \eqn{a_k^\top x}{a_k^\top x}
#' and its target value \eqn{g_k}{g_k}.
#'
...
#' \code{objective_term()}, and \eqn{x}{x} denotes the vector of
#' item-module-panel decision variables.
#'
#' Each objective term has a user-specified target value \eqn{g_k}{g_k}.
#'
...
#'   Introduces a common non-negative deviation variable \eqn{d \ge 0}{d \ge 0} such that
#'   \deqn{
#'     |a_k^\top x - g_k| \le d.
#'   }
#'
...
#'   \eqn{d^+ \ge 0}{d^+ \ge 0} and \eqn{d^- \ge 0}{d^- \ge 0}, with
#'   \deqn{
#'     a_k^\top x - g_k = d^+ - d^-.
#'   }
#' }
...
#' **2. Optimization Form**
#'
#' Introduce one/two continuous auxiliary variable \eqn{d}{d} or \eqn{d^+,d^-}{d^+,d^-} representing the
...
#'
#' \deqn{
#'   |a_k^\top x - g_k| \le d,  k = 1, \ldots, K.
#' }
#'
#' **two_dev**
#'
#' \deqn{a_k^\top x + d^- \le  g_k ,  k = 1, \ldots, K}
#'
#' \deqn{a_k^\top x -d^+  \le g_k,  k = 1, \ldots, K}
#'
