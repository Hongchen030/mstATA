...
#' imposed through a user-specified upper bound \eqn{\delta}{\delta}--the capped maximin
#' strategy treats \eqn{\delta}{\delta} as a decision variable and optimizes it
#' jointly with the common floor value \eqn{y}{y}. The resulting objective to be maximized,
#' \eqn{(y - \delta)}{(y - \delta)}, balances improving the worst-performing objective
...
#' }
#'
#' where \eqn{a_k}{a_k} is the coefficient vector constructed by
#' \code{objective_term()}, and \eqn{x}{x} denotes the binary
...
#' }
#'
#' Unlike the classical maximin strategy (where \eqn{\delta}{\delta} is user-specified or
#' infinite), capped maximin treats \eqn{\delta}{\delta} as a **decision variable** and
#' optimizes it jointly with the common floor.
#'
#' Each objective term is created using \link{objective_term}() and may differ
...
#' (3) Objective type
#'
#' The capped maximin strategy supports **relative objectives only**: maximize \eqn{a_k^\top x}{a_k^\top x}
#'
#' Absolute (goal-based) objectives of the form
#' \eqn{|a_k^\top x - g_k|}{|a_k^\top x - g_k|} are **not permitted** under capped maximin.
#'
#'**2. Proportions and normalization**
#'
#' Each objective term may be associated with a positive proportion
#' \eqn{p_k > 0}{p_k > 0}. These proportions encode the relative scaling of each objective.
#'
#' By default, \eqn{p_k = 1}{p_k = 1} for all terms.
#'
#' **3. Overflow control via \eqn{\delta}{\delta}**
#'
#' The capped maximin formulation introduces a common overflow variable
#' \eqn{\delta \ge 0}{\delta \ge 0}, yielding the constraints:
#'
#' \deqn{
...
#'         \eqn{y}{y}, and
#'   \item no term exceeds the highest value by more than \eqn{\delta}{\delta}.
#' }
#'
#'
#' @section Mathematical Formulation:
#'
#' Let \eqn{x}{x} denote the vector of binary item-module decision variables.
#' For each objective term \eqn{k = 1,\ldots,K}{k = 1,\ldots,K}, define:
#'
#' \deqn{
#'   y_k = a_k^\top x.
#' }
...
#'
#' \itemize{
#'   \item \eqn{y}{y} -- the common normalized floor value;
#'   \item \eqn{\delta}{\delta} -- the maximum allowable overflow above the floor.
#' }
#'
