...
#'     \strong{(number of category levels) x (number of modules specified)}
#'
#'   \item \strong{Pathway-level:} when \code{which_pathway} is provided and
#'         \code{which_module = NULL}
#'
#'    The number of constraints is
#'     \strong{(number of category levels) x (number of pathways specified)}
#' }
#'
...
#'
#' \itemize{
#'   \item Each stimulus has one designated pivot item \eqn{i^*_s}{i*s}.
...
#'
#'   \item The stacked operator, \eqn{\substack{\leq \\ \geq \\ =}}{\substack{\leq \\ \geq \\ =}} denotes that the specification may take the form of an upper bound, lower bound, or exact value.
#'
#'   \item \eqn{n_c^{stim,m}}{n_c^{stim,m}} specify the required number of stimuli from category level c in module m.
#'
#'   \item \eqn{n_c^{stim,r}}{n_c^{stim,r}} specify the required number of stimuli from category level c in pathway r.
#' }
#'
#'
