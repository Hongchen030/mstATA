...
#'   probabilities at a fixed ability level \eqn{\theta}{\theta} (Rows correspond
...
#'
#' Consider there are \eqn{I}{I} items. Let the score for item
#' \eqn{i}{i} be a discrete random variable \eqn{Y_i}{Y_i} taking values
#' \eqn{\{0, 1, \dots, m\}}{\{0, 1, \dots, m\}}. Conditional on ability \eqn{\theta}{\theta},
...
#' \deqn{
#' E(S \mid \theta)
#'   = E(\sum_{i=1}^{I} Y_i \mid \theta)
#'   = \sum_{i=1}^{I} E(Y_i \mid \theta),
#' }
#'
#' where \eqn{S = \sum_{i=1}^{I} Y_i}{S = sum_{i=1}^I Y_i} denotes the total score.
#'
#' This function evaluates the above expression numerically using the
#' item category probability matrix produced by
#' \link{compute_icc}(). Missing categories (if any) are assumed
...
#' }
#'
#' where \eqn{p_{ik}}{p_{ik}(\theta)} denotes the probability that item \eqn{i}{i}
#' produces score \eqn{k}{k} at ability level \eqn{\theta}{\theta}.
#'
#' (2) If \code{icc} is a list, the expected score function
#' \eqn{E(S \mid \theta)}{E(S \mid \theta)} is first evaluated on the grid encoded in
...
#' \eqn{E(S \mid \theta)}{E(S \mid \theta)} at the specified ability level.
#'
#' @examples
