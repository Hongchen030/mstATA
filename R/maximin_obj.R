...
#' \eqn{y}{y} across \eqn{K}{K} linear objective scores \eqn{u_k = a_k^\top x}{u_k = a_k^\top x}.
#'
#' The compiled constraints enforce:
#'
#' \deqn{
#'   a_k^\top x \ge p_k y, k = 1,\dots,K,
#' }
#'
#' and, if an overflow bound \code{delta} is finite for term \eqn{k}{k},
#'
#' \deqn{
...
#' Supported fields:
#' \itemize{
#'   \item \code{proportions} -- A positive numeric vector of
#'   length(multiple_terms) specifying the relative scaling
#'   of each objective term. Defaults to a vector of ones.
#'
#'   \item \code{delta} -- A nonnegative numeric value controlling the
#'   overflow band. May be either a scalar or a numeric vector of
#'   length(multiple_terms). Each element must be strictly positive
...
#' }
#'
#' constructed by \link{objective_term}().
#'
...
#'
#' \itemize{
#'   \item \strong{Relative objective}: maximize \eqn{a_k^\top x}{a_k^\top x}.
#' }
#'
#' Absolute (goal-based) objectives of the form
#' \eqn{|a_k^\top x - g_k|}{|a_k^\top x - g_k|} are not permitted under the maximin strategy.
#'
#' **2.  Proportions and normalization**
#'
#' Each objective term may be associated with a positive proportion \eqn{p_k}{p_k}.
#' These proportions encode the relative scaling of each objective.
#'
...
#'
#'   \deqn{
#'     p_k   y \le a_k^\top x \le p_k   y + \delta_k, k = 1, \ldots, K.
#'   }
#'
...
#'
#' Let \eqn{x}{x} denote the vector of binary item-module-panel decision variables.
...
#' }
#'
#' Introduce a continuous auxiliary variable \eqn{y}{y} representing the minimum
...
#'
#' \deqn{
#'   y_k \ge p_k  y,  k = 1,\ldots,K,
#' }
#'
#' and optionally,
#'
#' \deqn{
#'   y_k \le p_k y + \delta_k,  k = 1,\ldots,K.
#' }
#'
#'
