...
#'
#' \deqn{
#'   \max   \min_{k = 1,\ldots,K} ( \frac{a_k^\top x}{p_k} ),
#' }
#'
#' where each objective term is a linear score \eqn{a_k^\top x}{a_k^\top x}, and
#' \eqn{p_k > 0}{p_k > 0} is a user-specified (or default) proportions that scales the
#' relative importance of term \eqn{k}{k}.
#'
...
#'
#' Let there be \eqn{K}{K} objective terms, each defined as a linear score:
#'
#' \deqn{
...
#'
#' \itemize{
#'   \item \eqn{a_k}{a_k} is the coefficient vector constructed by
#'         \code{objective_term()};
#'   \item \eqn{x}{x} denotes the binary item-module-panel decision variables;
#'   \item \eqn{p_k > 0}{p_k > 0} is the proportions used to normalize objective \eqn{k}{k}.
#' }
#'
#' The maximin strategy introduces a common auxiliary variable \eqn{y}{y} representing
...
#'
#' \deqn{
#'   \frac{a_k^\top x}{p_k} \ge y,  k = 1, \ldots, K.
#' }
#'
...
#'   exceeding the common minimum by more than \eqn{\delta_k}{\delta_k}, improving numerical
