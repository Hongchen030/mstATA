...
#' @description
#'
#' Compute the conditional score distributions in a module given
#' one or more ability values (\eqn{\theta}{\theta}).
#'
#' This function assumes that item category probabilities have already been
#' computed and are provided on a common category scale.
#'
...
#'   ability value \eqn{\theta}{\theta} and must be a numeric matrix with rows
...
#' @details
#'
#' Consider a module consisting of \eqn{I}{I} items. Let the score for item
#' \eqn{i}{i} be a discrete random variable \eqn{Y_i}{Y_i} taking values
#' \eqn{\{0,1,\dots,m_i\}}{\{0,1,\dots,m_i\}}. Conditional on ability \eqn{\theta}{\theta}, assume
#' local independence:
#'
#' \deqn{
...
#' }
#'
#' with support \eqn{\{0,\dots,\sum_i m_i\}}{\{0,\dots,\sum_i m_i\}}. For a fixed ability level
#' \eqn{\theta}{\theta}, the conditional distribution of \eqn{S}{S} is obtained by
...
#' }
#'
#' where \eqn{p_i = (p_{i0}(\theta),\dots,p_{im_i}(\theta))}{p_i = (p_{i0}(\theta),\dots,p_{im_i}(\theta))} and \eqn{*}{*}
#' denotes convolution.
#'
...
#' \eqn{s = 0,\dots,\sum_i m_i}{s = 0,\dots,\sum_i m_i}. Columns correspond to ability values
#' \eqn{\theta}{\theta}. Each column sums to one.
#'
#' @examples
