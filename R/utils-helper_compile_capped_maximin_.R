...
#'
#' Let there be \eqn{K}{K} objective terms, each defined by a linear score:
#'
#' \deqn{
...
#'   \item \eqn{p_k > 0}{p_k > 0} is a user-specified proportions that scales the relative
#'   importance of objective \eqn{k}{k}.
#' }
#'
#' The capped-maximin strategy introduces two auxiliary continuous variables:
#'
#' \itemize{
#'   \item \eqn{y}{y}, representing a common lower bound on the scaled objective values;
#'   \item \eqn{\delta}{\delta}, representing the maximum allowed overflow above this bound.
#' }
#'
...
#'
#' \itemize{
#'   \item The variable \eqn{y}{y} captures the *worst-case* normalized objective value
#'   across all terms.
#'
#'   \item The variable \eqn{\delta}{\delta} controls how much any objective is allowed to
#'   exceed its proportional share.
#'
#'   \item Maximizing \eqn{y - \delta}{y - \delta} simultaneously encourages fairness across
