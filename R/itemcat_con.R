...
#' error is thrown.
#'
#' For details about \code{item_module_eligibility}, see \link{mst_design}().
#'
...
#'
#' In specified module m: for all \eqn{i_s}{i_s} specified in \code{item_ids}
#'
#' \eqn{x_{i_s,m} = 1}{x(i_s,m)=1} if item \eqn{i_s}{i_s} must be selected in module m.
#'
#' \eqn{x_{i_s,m} = 0}{x(i_s,m)=0} if item \eqn{i_s}{i_s} must not be selected in module m.
#'
#'
#' **2. Pathway-level item selection/not selection**
#'
#' In specified pathway r:  for all \eqn{i_s}{i_s} specified in \code{item_ids}
#'
#' \eqn{\sum_{m \in r} x_{i_s,m} = 1}{sum_{m in r} x(i_s,m) = 1}
#' if item \eqn{i_s}{i_s} must be selected in pathway r.
#'
#' \eqn{\sum_{m \in r} x_{i_s,m} = 0}{sum_{m in r} x(i_s,m) = 0}
#' if item \eqn{i_s}{i_s} must not be selected in pathway r.
#'
#' **3. Panel-level item selection/not selection**
#'
#' In a panel: for all \eqn{i_s}{i_s} specified in \code{item_ids}
#'
#' \eqn{\sum_{m} x_{i_s,m} = 1}{sum_m x(i_s,m) = 1} if item \eqn{i_s}{i_s}
#' must be selected in a panel.
#'
#' \eqn{\sum_{m} x_{i_s,m} = 0}{sum_m x(i_s,m) = 0} if item \eqn{i_s}{i_s}
#' must not be selected in a panel.
#'
#' Here:
#' \itemize{
