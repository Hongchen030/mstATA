...
#'       variables; lower and upper bounds are set to \eqn{-\infty}{-\infty} and
#'       \eqn{\infty}{\infty}, respectively.}
#'   }
...
#'   corresponding element of \code{ub}. This argument must be \code{NULL} when
#'   \code{continuous_bound_source} is \code{"compiled"} or
#'   \code{"none"}.
...
solve_model <- function(model_spec,
  ...
  params <- switch(solver,
                   ...
                   HiGHS = list(threads = 4,time_limit = time_limit,mip_rel_gap = 1e-2,mip_abs_gap = 0.0,
                                ...
                                random_seed = 200),
                   lpsolve = list(presolve = "none",timeout = time_limit,
