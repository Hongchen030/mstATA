...
#' \eqn{q_s}{q_s} are **strictly positive**.
#'
...
#' logic using \code{item_module_eligibility()} inside \link{mst_design}(), which
...
#' between \eqn{-1}{-1} and \eqn{1}{1}, users should filter stimuli into the routing
...
#'
#' Let \eqn{q_s}{q_s} be the quantitative attribute value of stimulus \eqn{s}{s}.
#'
#' One-sided upper-bound constraint:
#'
#' \deqn{
#'   q_s \, x_{i_s^{*},m} \le b_q^{stim,max}, s = 1, \ldots, S - 1.
#' }
#'
#' One-sided lower-bound constraint:
#'
#' \deqn{
#'   b_q^{stim,min} \, x_{i_s^{*},m} \le q_s, s = 1, \ldots, S - 1.
#' }
#'
...
#'
#'  \item \eqn{q_s}{q_s} (**must be positive values**) denote the values of the quantitative attribute for stimulus s.
#'
#'  \item \eqn{b_q^{stim,min}}{b_q^{stim,min}} and \eqn{b_q^{stim,max}}{b_q^{stim,max}} are the lower and
#' upper allowable bounds for the attribute \eqn{q_s}{q_s}.
#' }
#'
