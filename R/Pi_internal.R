...
#' at a given ability value \eqn{\theta}{\theta}.
#'
...
#' discrimination (\eqn{a_j}{a_j}), difficulty (\eqn{b_j}{b_j}),
#' guessing (\eqn{c_j}{c_j}),upper (\eqn{d_j}{d_j}).
#'
#'
#' (2) Polytomous IRT models
#'
#' For a polytomous item j, let \eqn{g_j}{g_j} denote the maximum score,
#' so that the possible response categories are
#' \eqn{0, 1, \ldots, g_j}{0, 1, \ldots, g_j}.
#'
...
#' \itemize{
#'   \item \strong{GRM}:
#'   A matrix with \eqn{\max_j g_j + 1}{max_j g_j + 1} columns, containing parameters
#'   \eqn{(\alpha_j, \beta_{j1}, \ldots, \beta_{j g_j})}{(alpha_j, beta_j1, ..., beta_jg_j)},
#'   where \eqn{\alpha_j}{alpha_j} is the discrimination parameter and
#'   \eqn{\beta_{jk}}{beta_jk} are the ordered threshold parameters.
#'
#'   \item \strong{MGRM}:
#'   A matrix with \eqn{g_j + 2}{g_j + 2} columns, containing parameters
#'   \eqn{(\alpha_j, b_j, c_{j1}, \ldots, c_{j g_j})}{(alpha_j, b_j, c_j1, ..., c_jg_j)},
#'   where \eqn{\alpha_j}{alpha_j} is the discrimination parameter,
#'   \eqn{b_j}{b_j} is the location parameter, and
#'   \eqn{c_{jk}}{c_jk} are category step parameters.
#'
#'   \item \strong{PCM}:
#'   A matrix with \eqn{\max_j g_j}{max_j g_j} columns, containing step parameters
#'   \eqn{(\delta_{j1}, \ldots, \delta_{j g_j})}{(delta_j1, ..., delta_jg_j)}.
#'
#'   \item \strong{GPCM}:
#'   A matrix with \eqn{\max_j g_j + 1}{max_j g_j + 1} columns, containing parameters
#'   \eqn{(\alpha_j, \delta_{j1}, \ldots, \delta_{j g_j})}{(alpha_j, delta_j1, ..., delta_jg_j)},
#'   where \eqn{\alpha_j}{alpha_j} is the discrimination parameter and
#'   \eqn{\delta_{jk}}{delta_jk} are step parameters.
#'
#'   \item \strong{RSM}:
#'   A matrix with \eqn{g + 1}{g + 1} columns, containing parameters
#'   \eqn{(\lambda_j, \delta_1, \ldots, \delta_g)}{(lambda_j, delta_1, ..., delta_g)},
#'   where \eqn{\lambda_j}{lambda_j} is the item location parameter and
#'   \eqn{\delta_k}{delta_k} are common step parameters across items.
#'
#'   \item \strong{NRM}:
#'   A matrix with \eqn{2 \max_j g_j}{2 max_j g_j} columns, containing parameters
#'   \eqn{(\alpha_{j1}, c_{j1}, \alpha_{j2}, c_{j2}, \ldots,
#'   \alpha_{j g_j}, c_{j g_j})}{(alpha_j1, c_j1, alpha_j2, c_j2, ..., alpha_jg_j, c_jg_j)},
#'   where \eqn{\alpha_{jk}}{alpha_jk} and \eqn{c_{jk}}{c_jk} are the slope and intercept
#'   parameters for category \eqn{k}{k}.
#' }
#'
...
#'   \eqn{\partial P_{jk}}{\partial P_{jk}(\theta)/\partial \theta}.}
#'
#' }
#'
#' @export
