...
#'     \itemize{
#'       \item `"Module-level"` implies stimulus must be selected/not selected in the
#'              specified module(s).
#'       \item `"Pathway-level"` implies stimulus must be selected/not selected in one
#'              of the modules composing that pathway.
#'       \item `"Panel-level"` implies stimulus must be selected/not selected somewhere in the
...
#'
#' In specified module m: for all \eqn{s}{s} (s= 1,2...,S-1) specified in \code{stim_ids}
#'
#' \deqn{x_{i_s^{*},m} = 1  (selected)}
#'
#' \deqn{x_{i_s^{*},m} = 0  (not selected)}
#'
#' **2. Pathway-level stimulus selection/not selection**
#'
#' In specified pathway r: for all \eqn{s}{s} (s= 1,2...,S-1) specified in \code{stim_ids}
#'
#' \deqn{\sum_{m \in r} x_{i_s^{*},m} = 1  (selected)}
#'
#' \deqn{\sum_{m \in r} x_{i_s^{*},m} = 0  (not selected)}
#'
#'
#' **3. Panel-level stimulus selection/not selection**
#'
#' In a panel: for all \eqn{s}{s} (s= 1,2...,S-1) specified in \code{stim_ids}
#'
#' \deqn{\sum_{m} x_{i_s^{*},m} = 1  (selected)}
#'
#' \deqn{\sum_{m} x_{i_s^{*},m} = 0  (not selected)}
#'
#' Here:
#' \itemize{
