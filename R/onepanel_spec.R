...
#' single panel. This includes requirements defined at the item, stimulus,
...
#' are detected, and \link{stim_itemcount_con}() is not included,
#' \code{onepanel_spec()} automatically injects item-stimulus gating constraints
#' via all-in/all-out selection. This ensures that item-level and stimulus-level selections remain
...
onepanel_spec <- function(x,constraints,objective = NULL) {
  ...
  if (needs_dvlink_item_stim(constraints)) {
    ...
    constraints <- c(constraints,
                     list(stim_itemcount_con(x = x,min = NULL,max = NULL)))
  }
