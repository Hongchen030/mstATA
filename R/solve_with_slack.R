...
#'
#' \itemize{
#'   \item For \code{"<="} rows, the slack enters with coefficient \eqn{-1}{-1}.
#'   \item For \code{">="} rows, the slack enters with coefficient \eqn{+1}{+1}.
#' }
#'
...
#' indexed by \eqn{b = 1,\dots,B}{b = 1,\dots,B}. Each block \eqn{b}{b} corresponds to one row in
#' the specification table and may contain multiple linear constraint rows
#' indexed by \eqn{r \in \mathcal{R}}{r \in \mathcal{R}_b}.
#'
#' For each constraint row, let \eqn{a_r^\top x}{a_r^\top x} denote the left-hand side
#' linear expression and \eqn{d_r}{d_r} the right-hand side constant.
#'
#' \strong{Soft constraint blocks and slack variables.}
#' For each relaxable constraint block \eqn{b}{b} (categorical or quantitative),
#' a single nonnegative continuous slack variable \eqn{s_b \ge 0}{s_b \ge 0} is introduced
#' and shared across all rows in that block.
#'
#' \strong{Original (hard) constraints.}
#' For a row \eqn{r \in \mathcal{R}}{r \in \mathcal{R}_b}, the original linear constraint takes one
...
#' }
#'
#' Equality constraints (\eqn{a_r^\top x = d_r}{a_r^\top x = d_r}) are not relaxable and therefore
#' are excluded from slack construction.
#'
#' \strong{Relaxed constraints with block-level slack.}
#' For a relaxable block \eqn{b}{b}, each row \eqn{r \in \mathcal{R}}{r \in \mathcal{R}_b} is modified
#' as follows:
#'
#' \itemize{
#'   \item If the original row is \eqn{a_r^\top x \le d_r}{a_r^\top x \le d_r}, the relaxed form is
#'     \deqn{a_r^\top x \le d_r + s_b.}
#'
#'   \item If the original row is \eqn{a_r^\top x \ge d_r}{a_r^\top x \ge d_r}, the relaxed form is
#'     \deqn{a_r^\top x \ge d_r - s_b.}
#' }
#'
#' Equivalently, in matrix form, the slack variable \eqn{s_b}{s_b} enters each row
#' with coefficient \eqn{-1}{-1} for \code{"<="} constraints and \eqn{+1}{+1} for
...
#' Let \eqn{\mathcal{B}_{cat}}{\mathcal{B}_{cat}} and \eqn{\mathcal{B}_{quant}}{\mathcal{B}_{quant}} denote
...
#' \deqn{
#' \min   f(x)
#' + \sum_{b \in \mathcal{B}_{cat}} \lambda_{cat}\, s_b
#' + \sum_{b \in \mathcal{B}_{quant}} \lambda_{quant}\, s_b,
#' }
#'
#' where \eqn{f(x)}{f(x)} denotes the original objective function, and
#' \eqn{\lambda_{cat}}{\lambda_{cat} > \lambda_{quant} > 0} are user-specified
