...
#'     \strong{(number of items) x (number of modules specified) x side}
#'
#'   \item \strong{Pathway-level:} when \code{which_pathway} is provided and
#'         \code{which_module = NULL}
#'
#'  The number of constraints is
#'     \strong{(number of items) x (number of unique modules in specified pathways) x side}
#'
#'   \item \strong{Panel-level:} both \code{which_module} and \code{which_pathway} are \code{NULL}
#'
#'  The number of constraints is
#'     \strong{(number of items) x (total number of modules) x side}
#' }
#'
...
#' value \eqn{q_{i_s}}{q_{i_s}}. If \eqn{q_{i_s}}{q_{i_s}} is zero or negative, the resulting
...
#'
#' **Instead, per-item quantitative rules should be enforced during the design stage**
#' using \code{item_module_eligibility()} within \link{mst_design}(),
...
#'
#'  \item \eqn{q_{i_s}}{q_{i_s}} (**must be positive values**) denote the values of a quantitative attribute for item \eqn{i_s}{i_s}.
#'
#'  \item \eqn{b_q^{item,min}}{b_q^{item,min}} and \eqn{b_q^{item,max}}{b_q^{item,max}} are the lower and
#' upper allowable bounds for the attribute \eqn{q_{i_s}}{q_{i_s}}.
#' }
#'
#' @return
