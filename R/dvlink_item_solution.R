...
#'
#' @description
#' This function does **not** need to be called by users and is internally used by \link{multipanel_spec}().
#' It is a **structural component** of multi-panel MST assembly.
#'
#' It has two roles.
#'
#' - Define the semantics of the solution-level indicator
#' variable \eqn{s_i}{s_i}: an item is considered *used* if and only if it is selected
...
#' \eqn{2 \times P}{2*P} where \eqn{P}{P} is the pool size,
...
#' indexed by \eqn{i^*_s}{i^*_s}. In addition, the pool contains a set of discrete
...
#' \eqn{m = 1, \ldots, M}{m = 1, \ldots, M} denote the module index. Suppose
#' there are \eqn{P}{P} panels assembled in one solution. Let
#' \eqn{p = 1, \ldots, P}{p = 1, \ldots, P} denote the panel index.
#'
#' **Decision variable set 1: item-module-panel selection variables**
#'
#' For each item \eqn{i_s}{i_s}, \eqn{x_{i_s,m,p}}{x(i_s,m,p)} = 1 if item
#' \eqn{i_s}{i_s} is selected in module \eqn{m}{m} of panel \eqn{p}{p}.
#' \eqn{x_{i_s,m,p}}{x(i_s,m,p)} = 0 otherwise.
#'
#'
#' **Decision variable set 2: solution-level item indicators**
#'
#' For each item \eqn{i_s}{i_s}, \eqn{s_{i_s}}{s_{i_s}} = 1 if item
#' \eqn{i_s}{i_s} is used in any module in any panel.
#' \eqn{s_{i_s}}{s_{i_s}} = 0 otherwise.
#'
#'
#' **Number of times item i is selected in a solution**
#'
#' \deqn{u_i = \sum_{p}\sum_{m} x_{i_s,m,p}}
#'
#' This constraints enforce:
#'
#' \deqn{m_i^{\min} s_{i_s} \le u_i \le m_i^{\max} s_{i_s}}
#'
...
#' where:
#' \itemize{
#'   \item \eqn{x_{i_s,m,p}}{x_{i_s,m,p}} indicates whether item \eqn{i_s}{i_s} is selected in module
#'         \eqn{m}{m} of panel \eqn{p}{p};
#'   \item \eqn{s_{i_s}}{s_{i_s}} indicates whether item \eqn{i_s}{i_s} is used anywhere in the solution;
#'   \item \eqn{m_i^{\min}}{m_i^{\min}} and \eqn{m_i^{\max}}{m_i^{\max}} are item-specific
#'         reuse bounds.
#' }
#'
...
#'
#' \itemize{
#'   \item If \eqn{s_i = 0}{s_i = 0}, then item \eqn{i}{i} cannot be selected in any module
#'         of any panel.
#'   \item If \eqn{s_i = 1}{s_i = 1}, the total number of selections of item \eqn{i}{i}
#'         must lie within the specified reuse bounds.
#'   \item If item \eqn{i}{i} is not selected in any module across panels,
#'         then \eqn{s_i}{s_i} must equal 0.
#'   \item If item \eqn{i}{i} is selected in at least one module of any panel,
#'         then \eqn{s_i}{s_i} must equal 1.
#' }
#'
#'
#'
