...
#'   pathways. Each element must be a string of the form "x-y-z", where
...
#'   \link{create_pivot_stimulus_map}(). Required for stimulus-based constraints.
#' @param enemyitem_set Optional. A precomputed enemy item set created by
#'   \link{create_enemy_sets}() and optionally combined via
#'   \link{concat_enemy_sets}(). Required for enemy-item constraints.
#' @param enemystim_set Optional. A precomputed enemy stimulus set created by
#'   \link{create_enemy_sets}() and optionally combined via
#'   \link{concat_enemy_sets}(). Required for enemy-stimulus constraints.
#'
...
#' \eqn{x_{i,m}}{x_{i,m}} indicating whether item \eqn{i}{i} is selected in module \eqn{m}{m}
#' in a panel. Panel-level item exposure constraints ensure uniqueness across modules within a pathway/panel.
#'
...
#' preserving flexibility near module boundaries and avoids rigid item partitioning in original
#' hybrid assembly strategy (Xiong, 2018).
#'
#'
#' @seealso
