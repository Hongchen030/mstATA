...
#'
#' @description
#' Implements a **minimax (goal-based)** objective that minimizes a common
#' ceiling on deviations from multiple target values.
#'
#' Let there be \eqn{K}{K} objective terms with linear scores
#' \eqn{y_k = a_k^\top x}{y_k = a_k^\top x} and target values \eqn{g_k}{g_k}.
#' The minimax objective minimizes the *largest deviation* from these targets
#' across all terms.
#'
...
#' All objective terms must be **absolute (goal-based) objectives**
...
#' }
#'
#' where \eqn{a_k}{a_k} is a coefficient vector defined by the attribute,
#' attribute level, and application scope, and \eqn{x}{x} denotes the
#' item-module-panel decision variables.
#'
#' Each term has an associated target value \eqn{g_k}{g_k}.
#'
#' The minimax strategy seeks a solution that minimizes the *worst-case*
...
#'
#'   Introduces a single nonnegative deviation variable \eqn{d \ge 0}{d \ge 0}
#'   representing the **largest absolute deviation** across all objectives.
#'
...
#'   \eqn{d^+ \ge 0}{d^+ \ge 0} (maximum overflow) and
#'   \eqn{d^- \ge 0}{d^- \ge 0} (maximum shortage).
#'
#'   For each objective term:
#'
#'   \deqn{a_k^\top x - g_k \le d^+}
#'
#'   \deqn{g_k - a_k^\top x \le d^-}
#'
#'   The optimization problem is:
#'
#'   \deqn{
...
#'
#' \itemize{
#'   \item \eqn{d}{d} (one-dev) measures the largest absolute deviation from any target;
#'   \item \eqn{d^+}{d^+} measures the largest amount by which any objective exceeds its target;
#'   \item \eqn{d^-}{d^-} measures the largest amount by which any objective falls short of its target.
#' }
#'
#' @section Mathematical Formulation:
#'
#' For \eqn{k = 1, \ldots, K}{k = 1, \ldots, K}, let
#'
#' \deqn{
#'   y_k = a_k^\top x,
#' }
...
#'
#' \itemize{
#'   \item \eqn{a_k}{a_k} is the coefficient vector defined by the objective term;
...
minimax_obj<-function(x,multiple_terms,strategy_args = list()){
  ...
  return(compile_objective(x = x,obj_set = multiple_terms,
                           strategy = "minimax",strategy_args = strategy_args))
}
